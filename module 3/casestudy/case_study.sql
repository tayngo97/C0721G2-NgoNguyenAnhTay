...
NgayKetThuc date,
TienDatCoc int,
constraint pk_IDnhanvien foreign key(IDNhanVien) references NhanVien(IDNhanVien) on delete set null,
constraint pk_IDkhachang foreign key(IDKhachHang) references khachhang(IDKhachHang) on delete set null,
constraint pk_IDdichvu foreign key(IDDichVu) references DichVu(IDDichVu) on delete set null
);
